...
unsigned long Ultrasonic::ping_cm() {
  unsigned long echoTime = Ultrasonic::ping();         // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
...
unsigned long Ultrasonic::ping_in() {
  unsigned long echoTime = Ultrasonic::ping();         // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
...
unsigned long Ultrasonic::ping_median(uint8_t it) {
...
void Ultrasonic::timer_ms(unsigned long frequency, void (*userFunc)(void)) {
  ...
  TIMSK4 = (1<<TOIE4);   // Enable Timer4 interrupt.
#elif defined (__arm__) && defined (TEENSYDUINO) // Timer for Teensy 3.x
  itimer.begin(Ultrasonic::timer_ms_cntdwn, 1000);  // Set timer to 1ms (1000 uS).
#else
