...
Touch::Touch (uint8_t cs_pin, uint8_t irq_pin) 
: _cs_pin(cs_pin), _irq_pin(irq_pin) {
}

void Touch::begin(uint16_t width, uint16_t height) {
...
void Touch::getCalibrationPoints(uint16_t &x1, uint16_t &y1, uint16_t &x2, uint16_t &y2) {
  x1 = y1 = CAL_MARGIN;
  x2 = _width - CAL_MARGIN;
  y2 = _height - CAL_MARGIN;
}

void Touch::setCalibration (uint16_t vi1, uint16_t vj1, uint16_t vi2, uint16_t vj2) {
...
uint16_t Touch::_readLoop(uint8_t ctrl, uint8_t max_samples) const {
...
void Touch::getRaw (uint16_t &vi, uint16_t &vj, adc_ref_t mode, uint8_t max_samples) const {
...
void Touch::getPosition (uint16_t &x, uint16_t &y, adc_ref_t mode, uint8_t max_samples) const {
  if (!isTouching()) {
    x = y = 0xffff;
    return;
  }
