...
LCD::LCD(uint8_t cs, uint8_t dc, uint8_t mosi,
           uint8_t sclk, uint8_t rst, uint8_t miso) : Graphics(ILI9340_TFTWIDTH, ILI9340_TFTHEIGHT) {
  _cs   = cs;
  _dc   = dc;
  _mosi  = mosi;
  _miso = miso;
...
LCD::LCD(uint8_t cs, uint8_t dc, uint8_t rst) : Graphics(ILI9340_TFTWIDTH, ILI9340_TFTHEIGHT) {
  _cs   = cs;
  _dc   = dc;
  _rst  = rst;
  hwSPI = true;
...
void LCD::commandList(uint8_t *addr) {
...
void LCD::setRotation(uint8_t m) {
...
void LCD::invertDisplay(boolean i) {
...
uint8_t LCD::readcommand8(uint8_t c) {
   digitalWrite(_dc, LOW);
   digitalWrite(_sclk, LOW);
   digitalWrite(_cs, LOW);
   spiwrite(c);
